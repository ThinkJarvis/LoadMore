...
public class PagerGridLayoutManager extends LayoutManager implements ScrollVectorProvider {
     ...
     */
    public PagerGridLayoutManager(@IntRange(from = 1, to = 100) int rows, @IntRange(from = 1, to
            = 100) int columns, @OrientationType int orientation) {
    ...
    public void onItemsUpdated(RecyclerView recyclerView, int positionStart, int itemCount) {
        ...
        for (int i = positionStart; count > 0; i++, count--) {
            if (i >= adapter.getItemCount()) {
                continue;
            }
            ViewHolder viewHolder = recyclerView.findViewHolderForAdapterPosition(i);
            if (viewHolder == null) {
                viewHolder = adapter.onCreateViewHolder(recyclerView, adapter.getItemViewType(i));
            }
    ...
    private void recycleAndFillItems(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        // 计算显示区域区前后多存储一列或则一行
        Rect displayRect = new Rect(mOffsetX - mItemWidth, mOffsetY - mItemHeight, getUsableWidth
                () + mOffsetX + mItemWidth, getUsableHeight() + mOffsetY + mItemHeight);
    ...
    private void addOrRemove(RecyclerView.Recycler recycler, Rect displayRect, int i) {
        ...
        if (!Rect.intersects(displayRect, rect)) {
            ...
            layoutDecorated(child, rect.left - mOffsetX + lp.leftMargin + getPaddingLeft(), rect
                    .top - mOffsetY + lp.topMargin + getPaddingTop(), rect.right - mOffsetX - lp
                    .rightMargin + getPaddingLeft(), rect.bottom - mOffsetY - lp.bottomMargin +
                    getPaddingTop());
        }
    }
    ...
    public void onMeasure(RecyclerView.Recycler recycler, RecyclerView.State state, int
        ...
        setMeasuredDimension(View.MeasureSpec.makeMeasureSpec(widthsize, widthmode), View
    ...
    private int[] getPageLeftTopByPosition(int pos) {
        ...
        int page = getPageIndexByPosition(pos);
        if (canScrollHorizontally()) {
    ...
    public void smoothScrollToPosition(RecyclerView recyclerView, RecyclerView.State state, int
            position) {
        int targetPageIndex = getPageIndexByPosition(position);
    ...
    public void scrollToPage(int pageIndex) {
        if (pageIndex < 0 || pageIndex >= mLastPageCount) {
            Log.e(TAG, "pageIndex = " + pageIndex + " is out of bounds, mast in [0, " +
                    mLastPageCount + ")");
            return;
        }
    ...
    public int getCurrentPageFirstPosition() {
        ...
        return mLastPageIndex * mOnePageSize;
    }
    ...
    public int getCurrentPageLastPosition() {
        ...
        int position = (mLastPageIndex + 1) * mOnePageSize - 1;
        int max = getTotalPageCount() * mOnePageSize - 1;
        if (position > max) {
            position = max;
        }
        return position;
