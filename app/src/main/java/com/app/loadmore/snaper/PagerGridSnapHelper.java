...
public class PagerGridSnapHelper extends SnapHelper {
    private int mFlingThreshold = 1000;
    ...
    public int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX, int velocityY) {
        ...
        if (null != layoutManager && layoutManager instanceof PagerGridLayoutManager) {
            ...
            if (manager.canScrollHorizontally()) {
                if (velocityX > mFlingThreshold) {
                    target = manager.findNextPageFirstPos();
                } else if (velocityX < -mFlingThreshold) {
                    target = manager.findPrePageFirstPos();
                }
            } else if (manager.canScrollVertically()) {
                if (velocityY > mFlingThreshold) {
                    target = manager.findNextPageFirstPos();
                } else if (velocityY < -mFlingThreshold) {
                    target = manager.findPrePageFirstPos();
                }
            }
        }
    ...
    public boolean onFling(int velocityX, int velocityY) {
        ...
        int minFlingVelocity = mFlingThreshold;
        return (Math.abs(velocityY) > minFlingVelocity || Math.abs(velocityX) > minFlingVelocity) && snapFromFlingInner(layoutManager, velocityX, velocityY);
    }
     ...
     * @return 是否消费该事件
     */
    private boolean snapFromFlingInner(@NonNull RecyclerView.LayoutManager layoutManager, int velocityX, int velocityY) {
